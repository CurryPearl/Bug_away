...
mm=0
def uart_read():##可以读入并存为10位长度的
    global rec,page
    if uart.any():
        temp=list(uart.readline())
        j=0
        count_flag=0
        for i in range(0,len(temp)):
            if count_flag==0:
                ...
                if temp[i]!=115 and i<=len(temp):
                    commands[j]=int(temp[i]-48)
                    j+=1
                elif temp[i]==115:
                    count_flag=2
            elif count_flag==2:
...
def mode_change(mode,car_number):
    global guan_dian_count
    global CROSS_FLAG
    global WAY_ORDER
    global num_rec###数字识别结果
    #parameter_clear(),需要修改
    if car_number=="master":
        if mode==1:#只走外圈
            CROSS_FLAG=1
            WAY_ORDER=1
        elif mode==2 or mode==3:#主车先走外圈再走内圈
            if guan_dian_count<=1:
                CROSS_FLAG=1
                WAY_ORDER=1
            elif guan_dian_count==3:
                CROSS_FLAG=0
                WAY_ORDER=1
        elif mode==4:
            pass
        elif mode==5:
            if num_rec==1:  ##对应数字1，全外圈
                CROSS_FLAG=1
                WAY_ORDER=1
            if num_rec==2:  ##对应数字2，全内圈
                CROSS_FLAG=0
                WAY_ORDER=1
            if num_rec==3:  ##对应数字3，先内再外
                if guan_dian_count<=1:
                    CROSS_FLAG=0
                    WAY_ORDER=1
                elif guan_dian_count==3:
        ...
        if mode==1:
            ...
            if num_rec==1:  ##对应数字1，全内圈
                CROSS_FLAG=0
                WAY_ORDER=0
            if num_rec==2:  ##对应数字2，全外圈
                CROSS_FLAG=1
                WAY_ORDER=0
            if num_rec==3:  ##对应数字3，先外再内
                if guan_dian_count<=1:
                    CROSS_FLAG=1
                    WAY_ORDER=0
                elif guan_dian_count==3:
...
def search_mid(cross_flag,way_order):#当cross_flag为1时走外圈，为0时走内圈;当way_order为1时逆时针，为0时顺时针
    ...
    _threshold = int((average ) / 2.0+12)  # 计算阈值
    ...
    if count==2:#只需要判断count==2的情况，其他情况全部归于else即可
        if zzz-zzz_remember>1700:######注意这里的1500需要根据速度实时调整
            guan_dian_count+=1
            zzz_remember=zzz
...
while(True):
    ...
    ccd_get()
    uart_read()
    mode_change(page,"slave")
    cha=int(0.5*(search_mid(CROSS_FLAG,WAY_ORDER)-64))
    ...
    if abs(cha) > 9:
     ...
     uart.write(str_num)  # 将整数转换为字节型数据并发送
    uart.write('00')
    str_guai=str(guan_dian_count)
    if abs(guan_dian_count) > 9:
     uart.write(str_guai)  # 将整数转换为字节型数据并发送
    else:
     uart.write('0')  # 发送字节型数据 '0'
     uart.write(str_guai)  # 将整数转换为字节型数据并发送

    uart.write('0%d'%(num_rec))
    #parameter_clear()
